...
async function getToken(body) {
    try {
        ...
        if (!data.tokens) {
            ...
            localStorage.setItem(locationToken,JSON.stringify({accessToken :accessToken,email: data.email}) );
            const a = localStorage.getItem(locationToken); //ubicacion del access token
            window.location.replace("./index.html");
        }; 
    } catch (error) {
        Swal.fire({
